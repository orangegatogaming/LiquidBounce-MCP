    ...
    public static boolean isBot(final EntityLivingBase entity) {
        ...
        if(antiBot.duplicateInWorldValue.get()) {
            if (mc.theWorld.loadedEntityList.stream()
                    .filter(currEntity -> currEntity instanceof EntityPlayer && ((EntityPlayer) currEntity)
                            .getDisplayName().getUnformattedText().equals(((EntityPlayer) currEntity).getDisplayName().getUnformattedText()))
                    .count() > 1)
                return true;
        }
        ...
        if(antiBot.duplicateInTabValue.get()) {
            if (mc.getNetHandler().getPlayerInfoMap().stream()
                    .filter(networkPlayer -> entity.getDisplayName().getUnformattedText().equals(ColorUtils.stripColor(EntityUtils.getName(networkPlayer))))
                    .count() > 1)
                return true;
        }

        return entity.getDisplayName().getUnformattedText().isEmpty() || entity.getDisplayName().getUnformattedText().equals(mc.thePlayer.getDisplayName().getUnformattedText());
    }
