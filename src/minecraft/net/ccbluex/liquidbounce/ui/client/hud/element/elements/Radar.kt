...
class Radar(x: Double = 5.0, y: Double = 130.0) : Element(x, y) {
    ...
    override fun drawElement(): Border? {
        ...
        if (triangleMode) {
            ...
            worldRenderer.func_181668_a(GL_POINTS, DefaultVertexFormats.field_181706_f)
        ...
        for (entity in mc.theWorld.loadedEntityList) {
            if (entity != null && entity !== mc.thePlayer && EntityUtils.isSelected(entity, false)) {
                ...
                if (triangleMode) {
                    ...
                    worldRenderer.func_181662_b(((positionRelativeToPlayer.x / viewDistance) * size).toDouble(),
                            ((positionRelativeToPlayer.y / viewDistance) * size).toDouble(), 0.0)
                        .func_181666_a(color.red / 255.0f, color.green / 255.0f,
                                    color.blue / 255.0f, 1.0f).func_181675_d()
                }
    ...
    private fun createFovIndicator(angle: Float): VertexBuffer {
        ...
        worldRenderer.func_181668_a(GL_TRIANGLE_FAN, DefaultVertexFormats.field_181705_e)
        ...
        while (curr >= start) {
            worldRenderer.func_181662_b(cos(curr) * radius, sin(curr) * radius, 0.0).func_181675_d()
        ...
        worldRenderer.reset()
        safeVertexBuffer.func_181722_a(worldRenderer.byteBuffer)
